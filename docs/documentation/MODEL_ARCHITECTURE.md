...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                              INPUT SEQUENCE (49 tokens)                              │
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                     │
│  Position:   0       1────────────20      21       22──────47      48               │
│              │            │                │           │            │               │
│              ▼            ▼                ▼           ▼            ▼               │
│           ┌─────┐   ┌───────────┐     ┌─────┐   ┌───────────┐   ┌─────┐            │
│           │[CLS]│   │  Tabular  │     │[SEP]│   │  Weather  │   │[SEP]│            │
│           │     │   │  Tokens   │     │     │   │  Tokens   │   │     │            │
│           └─────┘   └───────────┘     └─────┘   └───────────┘   └─────┘            │
│              │       (20 tokens)         │       (26 tokens)       │               │
│              │            │              │            │            │               │
│  Type ID:    0            1              0         2/3/4           0               │
│           (special)   (tabular)      (special)   (weather)    (special)            │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                  │
│   [SEP]  ─────────────────────────────────────────────────────►  │
│     │    "Information bottleneck" between modalities             │
│     │    Forces modalities to summarize before cross-attention   │
│     │                                                            │
│     └──► Learnable parameter: (1, hidden_size)                   │
│          (same embedding reused at positions 21 and 48)          │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
│   ├──────────┼─────────────────┼───────┼────────────────────────────────────┤  │
│   │    0     │ [CLS], [SEP]    │   3   │ Special tokens (pooling, boundary) │  │
...
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                     │
│   Input: Token Sequence (batch, 49, hidden_size)                                    │
│        │                                                                            │
│        ▼                                                                            │
│   ╔═════════════════════════════════════════════════════════════════════════════╗  │
│   ║                       TRANSFORMER ENCODER                                    ║  │
...
│   │                          SequenceAssembly                                     │  │
│   │                                                                               │  │
│   │   [CLS] + tabular_tokens + [SEP] + weather_tokens + [SEP]                    │  │
│   │     │          │             │           │             │                      │  │
│   │     ▼          ▼             ▼           ▼             ▼                      │  │
│   │   type=0     type=1       type=0      type=2/3/4    type=0                   │  │
│   │                                                                               │  │
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         PARAMETER COUNT (hidden_size=256)                        │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   TOKENIZATION                                                                  │
│   ├── CLS token:              1 × 256 = 256                                     │
